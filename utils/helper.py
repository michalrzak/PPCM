...
def load_model(model, checkpoint, verbose=False):
...
def load_model_recursive(model, checkpoint, verbose=False):
    if checkpoint is None or checkpoint == "None":
        if verbose:
            print('No checkpoint provided for %s!' % model._get_name())
    else:
...
def dist_score(s, enc):
    if (len(enc.encode(s)) > 0):
        return find_ngrams(enc.encode(s), n=2) / len(enc.encode(s))
    else:
        return 0
    ...
    if (entailment):
                ...
                if len(d) > 5:
                    if ("Human Conversation" in d):
                        ...
                        if ("Human 1: " in d):
                            ...
                            conversation.append(
                                {"turn": i, "speaker": "Human 1", "text": text_turn.strip('\n').strip()})
                        else:
                            _, text_turn = d.split("Human 2: ")
                            conversation.append(
                                {"turn": i, "speaker": "Human 2", "text": text_turn.strip('\n').strip()})
                        i += 1
        ...
        if (args.all_starter):
            ...
            for i_c, conv in enumerate(conversation):
                for index in range(len(conv) - 2):
                    ...
                    if (context_tokens <= 70):
                        list_starters.append(
                            {"conversation": [conv[index]["text"], conv[index + 1]["text"]], "knowledge": None,
                             "gold": None})
                    # else:
            ...
            for conv in conversation:
                ...
                list_starters.append(
                    {"conversation": [conv[index]["text"], conv[index + 1]["text"]], "knowledge": None, "gold": None})
...
def make_logdir(args):
    ...
    logdir = os.path.join('runs',
                          f'{args.dataset}_{args.label}_{current_time}')  # current_time + '_' + socket.gethostname() + '_' + model_name)
